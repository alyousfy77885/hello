...
if(isset($_POST["login"])){
 ...
 $status = "Active now";
            $sql = mysqli_query($con, "UPDATE user_info SET status = '{$status}',ft=CURRENT_TIMESTAMP WHERE user_id={$_SESSION['uid']}");
				...
				if(!isset($BackToMyPage)) {
					...
					echo"<script type='text/javascript'>
					 location.href='store.php'
					  ...
					  echo "<script>window.alert('تم تسجيل الدخول بنجاح');
					   location.href='store.php'; </script>";
            ...
            if($count == 1){
                ...
                //we have created a cookie in login_form.php page so if that cookie is available means user is not login
                    ...
                    echo "<script> window.alert('تم تسجيل الدخول بنجاح');
					   location.href='admin/addproduct.php'; </script>";
                    exit;

                }else{
					  echo "<script>window.alert('خطا في اسم المستخدم او كلمه المرور');
					   location.href='store.php'; </script>";
                    //exit();
                }
